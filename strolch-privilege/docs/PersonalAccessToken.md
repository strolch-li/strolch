...
- `getPersonalAccessTokens(Certificate certificate, String username)`: List tokens for the given user. Requires privilege `PrivilegePersonalAccessTokenUser` for that user.
- `createPersonalAccessToken(Certificate certificate, String name, ZonedDateTime validFrom, ZonedDateTime validTo, Set<String> roles, Set<String> privileges)`: Create a new token for the current user. Requires `PrivilegePersonalAccessToken`.
- `createPersonalAccessToken(Certificate certificate, String username, String name, ZonedDateTime validFrom, ZonedDateTime validTo, Set<String> roles, Set<String> privileges)`: Create a new token for the specified user (e.g. by an administrator for a technical / service user). Requires `PrivilegePersonalAccessToken` and privilege `PrivilegePersonalAccessTokenUser` for that user.
  - The new token's privileges are a snapshot of the target user's *current* privileges at the time of creation, optionally filtered by the given roles and/or privileges. 
  - If both roles and privileges are null or empty, then all of the target user's current privileges are assigned to the token.
- `removePersonalAccessToken(Certificate certificate, String tokenId)`: Revoke a token. Requires privilege `PrivilegePersonalAccessTokenUser` for that token's owner if revoking another user's token.
...
Retrieves all tokens belonging to the authenticated user, or for a specified user if permitted.

- **Method**: `GET`
- **Path**: `/`
- **Query Parameters**:
  - `username` (String, optional): Username of the target user whose tokens should be listed. Defaults to the authenticated user if omitted.
...
Creates a new token with a specific name, validity period, and optional privilege subset for the authenticated user or for a specified target user (such as a technical / service user).
...
**Request Body Schema**:
- `username` (String, optional): Target username for whom the token is created. If omitted or null, defaults to the authenticated user. Creating tokens for other users requires administrative privilege on `PrivilegePersonalAccessTokenUser`.
...
- `roles` (Array<String>, optional): List of role names to restrict the token to (must be roles held by the target user).
- `privileges` (Array<String>, optional): List of specific privilege names to assign (must be privileges held by the target user).

**Request Example**:
```json
{
  "username": "technical_api_user",
...
- **Authorization & Scope**: Token operations are restricted to the token owner or administrators with explicit privileges:
  - Users can create, view, and revoke their own tokens if they possess `PrivilegePersonalAccessToken`.
  - Administrators can create, list, and revoke tokens on behalf of other users (such as headless technical/service users) if they possess the `PrivilegePersonalAccessTokenUser` privilege for the target username.
  - When creating a token for another user, effective privileges are strictly scoped to the target user's granted roles and privileges, preventing privilege escalation. Authenticators receive a `Certificate` containing only the restricted subset of privileges configured on the token.
