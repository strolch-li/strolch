 ...
 */
public class UserAccessPrivilege extends PrivilegePolicy {
	...
	protected boolean validateAction(PrivilegeContext ctx, Privilege privilege, Restrictable restrictable,
		...
		if (!(object instanceof Tuple tuple)) {
			String msg = Restrictable.class.getName() + getString("Privilege.illegalArgument.nontuple");
		...
		switch (privilegeName) {
			case PRIVILEGE_GET_USER, PRIVILEGE_ADD_USER, PRIVILEGE_REMOVE_USER, PRIVILEGE_MODIFY_USER -> {
				User oldUser = tuple.getFirst();
				User newUser = tuple.getSecond();

				if (privilegeName.equals(PRIVILEGE_ADD_USER)
						|| privilegeName.equals(PRIVILEGE_GET_USER)
						|| privilegeName.equals(PRIVILEGE_REMOVE_USER)) {
					DBC.INTERIM.assertNull("For " + privilegeName + " first/oldUser must be null!", oldUser);
				} else {
					DBC.INTERIM.assertNotNull("For " + privilegeName + " first/oldUser must not be null!", oldUser);
				}
				DBC.INTERIM.assertNotNull("For " + privilegeName + " second/newUser must not be null!", newUser);
			...
			case PRIVILEGE_SET_USER_LOCALE, PRIVILEGE_SET_USER_PASSWORD -> {
				User oldUser = tuple.getFirst();
				User newUser = tuple.getSecond();
			...
			default -> {
				String msg = getString("Privilege.userAccessPrivilege.unknownPrivilege");
