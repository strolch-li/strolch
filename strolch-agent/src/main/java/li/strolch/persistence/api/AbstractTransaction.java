...
public abstract class AbstractTransaction implements StrolchTransaction {
	...
	@Override
	public <T extends StrolchPolicy> T getPolicy(PolicyContainer element, Class<T> clazz, PolicyDef defaultDef) {
		PolicyDef policyDef = element.getPolicyDef(clazz, defaultDef);
	...
	private boolean hasElementInFilter(String key, Locator locator) {
		return this.objectFilter.hasElement(key, locator);
	}
	...
	@Override
	public void assertHasPrivilege(Restrictable restrictable) throws AccessDeniedException {
		DBC.PRE.assertNotNull("restrictable must not be null", restrictable);
		getPrivilegeContext().validateAction(restrictable);
	}
	...
	public void addOrUpdate(Resource resource) throws StrolchModelException {
		...
		if (hasResource(resource.getType(), resource.getId())) {
			li.strolch.utils.objectfilter.Operation operation = this.objectFilter.getOperation(Tags.RESOURCE,
					resource.getLocator());
	...
	public void addOrUpdate(Order order) throws StrolchModelException {
		...
		if (hasOrder(order.getType(), order.getId())) {
			li.strolch.utils.objectfilter.Operation operation = this.objectFilter.getOperation(Tags.ORDER,
					order.getLocator());
			if (operation != li.strolch.utils.objectfilter.Operation.REMOVE)
	...
	public void addOrUpdate(Activity activity) throws StrolchModelException {
		...
		if (hasActivity(activity.getType(), activity.getId())) {
			li.strolch.utils.objectfilter.Operation operation = this.objectFilter.getOperation(Tags.ACTIVITY,
					activity.getLocator());
			if (operation != li.strolch.utils.objectfilter.Operation.REMOVE)
